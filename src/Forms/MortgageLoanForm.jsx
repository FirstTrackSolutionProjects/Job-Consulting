...
import applyForMortgageLoanService from "../services/loanServices/mortgageLoanServices/applyForMortgageLoanService";
import { toast } from "react-toastify";

const fileFields = [
  "photo", "aadharFile", "panFile", "incomeproof", "companypan", "companytan", "cin", "gst", "msme", "tradeLicense", "foodLicense", "drugLicense", "electricityBill", "rentagreement", "deedagreement", "bankStatementsCurrent", "bankStatementsCurrentYear1", "bankStatementsCCYear1", "itr1", "itr2", "itr3", "computation1", "computation2", "computation3", "bankProof"
];
const requiredFiles = [
...
const MortgageLoanForm = () => {
    ...
    try {
      ...
      await Promise.all(
        fileFields.map(async (key) => {
          ...
          const filename = `loans/mortgage-loans/${key}-${uuidv4()}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await applyForMortgageLoanService(uploadedFormData);
        ...
        <div className="space-y-4">
          ...
          <div>
            <input
              ...
              value={formData.mortgageProperty}
              onChange={handleChange}
              placeholder="Property to Mortgage"
              required
              className="w-full p-2 border rounded"
            />
          </div>
